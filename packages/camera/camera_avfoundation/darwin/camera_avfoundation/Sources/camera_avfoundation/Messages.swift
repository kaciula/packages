...
enum PlatformCameraStabilizationMode: Int {
  /// No stabilization.
  case off = 0
...
enum PlatformAVCaptureDeviceType: Int {
  /// A built-in wide-angle camera device type.
  case builtInWideAngleCamera = 0
  /// A built-in camera device type with a shorter focal length than a wide-angle camera.
  case builtInUltraWideCamera = 1
  /// A built-in camera device type with a longer focal length than a wide-angle camera.
...
struct PlatformCameraDescription: Hashable {
  ...
  var availableStabilizationModes: [PlatformCameraStabilizationMode]
  var captureDeviceType: PlatformAVCaptureDeviceType
  ...
  static func fromList(_ pigeonVar_list: [Any?]) -> PlatformCameraDescription? {
    ...
    let availableStabilizationModes = pigeonVar_list[3] as! [PlatformCameraStabilizationMode]
    let captureDeviceType = pigeonVar_list[4] as! PlatformAVCaptureDeviceType
  ...
  static func == (lhs: PlatformCameraDescription, rhs: PlatformCameraDescription) -> Bool {
    ...
    return deepEqualsMessages(lhs.name, rhs.name) && deepEqualsMessages(lhs.lensDirection, rhs.lensDirection) && deepEqualsMessages(lhs.lensType, rhs.lensType) && deepEqualsMessages(lhs.availableStabilizationModes, rhs.availableStabilizationModes) && deepEqualsMessages(lhs.captureDeviceType, rhs.captureDeviceType)
  }
  ...
  func hash(into hasher: inout Hasher) {
    ...
    deepHashMessages(value: availableStabilizationModes, hasher: &hasher)
    deepHashMessages(value: captureDeviceType, hasher: &hasher)
  }
}
...
private class MessagesPigeonCodecReader: FlutterStandardReader {
  override func readValue(ofType type: UInt8) -> Any? {
    ...
    case 139:
      let enumResultAsInt: Int? = nilOrValue(self.readValue() as! Int?)
      if let enumResultAsInt = enumResultAsInt {
        return PlatformCameraStabilizationMode(rawValue: enumResultAsInt)
      }
      return nil
    case 140:
      let enumResultAsInt: Int? = nilOrValue(self.readValue() as! Int?)
      if let enumResultAsInt = enumResultAsInt {
        return PlatformAVCaptureDeviceType(rawValue: enumResultAsInt)
      }
      return nil
    case 141:
...
private class MessagesPigeonCodecWriter: FlutterStandardWriter {
  override func writeValue(_ value: Any) {
    if let value = value as? PlatformCameraLensDirection {
      ...
      super.writeByte(138)
      super.writeValue(value.rawValue)
    } else if let value = value as? PlatformCameraStabilizationMode {
      super.writeByte(139)
      super.writeValue(value.rawValue)
    } else if let value = value as? PlatformAVCaptureDeviceType {
      super.writeByte(140)
      super.writeValue(value.rawValue)
...
protocol CameraApi {
  ...
  func create(cameraName: String, settings: PlatformMediaSettings, stabilizationMode: PlatformCameraStabilizationMode, completion: @escaping (Result<Int64, Error>) -> Void)
  ...
  static func setUp(binaryMessenger: FlutterBinaryMessenger, api: CameraApi?, messageChannelSuffix: String = "") {
    ...
    if let api = api {
      createChannel.setMessageHandler { message, reply in
        ...
        let stabilizationModeArg = args[2] as! PlatformCameraStabilizationMode
        api.create(cameraName: cameraNameArg, settings: settingsArg, stabilizationMode: stabilizationModeArg) { result in
          switch result {
          case .success(let res):
            reply(wrapResult(res))
          case .failure(let error):
